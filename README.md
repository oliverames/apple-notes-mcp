...
This tool does not decode strokes: Notes' Paper bundle has no public reader, so the image is Notes' own rendering. The opt-in private helper's [`native-read-paper`](#native-read-paper) decodes strokes.
