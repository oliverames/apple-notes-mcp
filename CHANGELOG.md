...
## [Unreleased]
### Added
- **Transient AppleScript failures now retry by default.** The retry machinery (transient-error classification, exponential backoff) has existed since the retry option shipped, but the default was `maxRetries: 1`, so a momentarily busy or unresponsive Notes.app failed the tool call instantly. The default is now `2` (one retry after 1s), applied **only** to the transient classes (busy / not responding / lost connection / timeout) — "note not found" and friends still fail immediately. Restore the old fail-fast behavior with `APPLE_NOTES_MCP_MAX_RETRIES=1`.
- **Reliability env knobs: `APPLE_NOTES_MCP_TIMEOUT_MS`, `APPLE_NOTES_MCP_MAX_RETRIES`, `APPLE_NOTES_MCP_RETRY_DELAY_MS`.** The 30s AppleScript timeout and the retry policy were previously hard-coded (per-call options existed internally but nothing user-facing). All three follow the same validation pattern as `APPLE_NOTES_MCP_MAX_BUFFER`, work through the config-file fallback for hosts that strip `env`, and lose to explicit per-call options. Large libraries where a full-library search legitimately exceeds 30s can now raise the timeout instead of flaking.
- **`doctor` now checks the Node runtime's code signature.** An ad-hoc signed Node (typically Homebrew's) gets a new cdhash on every update, so macOS TCC silently revokes its Automation and Full Disk Access grants — the most common cause of "worked last week, permission denied today". The new check inspects `process.execPath` with `codesign` and warns with a pointer to `docs/NODE-RUNTIME-AND-TCC-PERMISSIONS.md` when the signature is ad-hoc.
