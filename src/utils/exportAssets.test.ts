...
describe("SidecarWriter", () => {
  ...
  it("writes generated bytes create-only, once per key", () => {
    const dir = join(out, "gen.assets");
    mkdirSync(dir);
    writeFileSync(join(dir, "Sketch.svg"), "existing");
    const writer = new SidecarWriter(dir, out);
    const svg = Buffer.from("<svg/>");
    const placed = writer.placeBytes(svg, "Sketch.svg", "image/svg+xml", "drawing:A");
    expect(placed).toEqual({ url: "gen.assets/Sketch-2.svg", mime: "image/svg+xml" });
    expect(readFileSync(join(dir, "Sketch.svg"), "utf8")).toBe("existing");
    expect(readFileSync(join(dir, "Sketch-2.svg"))).toEqual(svg);
    expect(writer.placeBytes(svg, "Sketch.svg", "image/svg+xml", "drawing:A")).toBe(placed);
    expect(writer.count).toBe(1);
    const refused = new SidecarWriter(join(NOTES_CONTAINER, "never-created")).placeBytes(
      svg,
      "x.svg",
      "image/svg+xml",
      "k"
...
describe("DataUrlWriter", () => {
  ...
  it("embeds generated bytes under the same per-asset and total limits", () => {
    const svg = Buffer.from("<svg/>");
    const writer = new DataUrlWriter(MAX_EMBED_BYTES, svg.length * 2);
    const placed = writer.placeBytes(svg, "d.svg", "image/svg+xml", "drawing:A");
    expect(placed).toEqual({
      url: `data:image/svg+xml;base64,${svg.toString("base64")}`,
      mime: "image/svg+xml",
    });
    expect(writer.placeBytes(svg, "d.svg", "image/svg+xml", "drawing:A")).toBe(placed);
    expect(writer.placeBytes(svg, "e.svg", "image/svg+xml", "drawing:B")).toMatchObject({
      mime: "image/svg+xml",
    });
    expect(writer.placeBytes(svg, "f.svg", "image/svg+xml", "drawing:C")).toEqual({
      error: "too-large",
    });
    expect(writer.count).toBe(2);
    expect(new DataUrlWriter(3).placeBytes(svg, "d.svg", "image/svg+xml", "k")).toEqual({
      error: "too-large",
    });
  });
});
